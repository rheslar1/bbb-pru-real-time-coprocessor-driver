...
ProjectProfile demoProfile() {
  return ProjectProfile{
      "bbb-pru-real-time-coprocessor-driver",
      ...
      MetricSpec{"motor_current_a", "A", 4.5, true},
      MetricSpec{"loop_jitter_us", "us", 250.0, true},
      false};
}

RuntimeSample nominalSample() {
  return RuntimeSample{1U, 2.02, 112.5, true, "host-ci-nominal"};
}

RuntimeSample unsafeSample() {
  return RuntimeSample{2U, 6.08, 337.5, true, "host-ci-limit-test"};
}

RuntimeSample offlineSample() {
  return RuntimeSample{3U, 2.02, 112.5, false, "host-ci-offline"};
}
